...
AS $function$
  SELECT 
    ...
    p.active, p.banker_name, p.finder_name,
    ...
    p.user_id, p.full_name, p.cpf, p.empresa, p.area, p.gestor,
    p.advisor_name, p.operacao_tipo,
    ...
    p.created_at,
    tr.convite_status, tr.convite_enviado_em, tr.convite_aceito_em,
    tr.convite_expira_em, tr.convite_cancelado_em, tr.convite_reenvios,
  ...
  SELECT 
    ...
    tr.full_name, tr.cpf, tr.empresa, tr.area, tr.gestor,
    tr.advisor_name, tr.operacao_tipo,
  ...
  WHERE NOT EXISTS (SELECT 1 FROM profiles p WHERE LOWER(TRIM(p.email)) = LOWER(TRIM(tr.email)))
