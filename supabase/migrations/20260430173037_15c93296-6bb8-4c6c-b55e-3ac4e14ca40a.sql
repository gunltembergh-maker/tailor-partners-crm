...
WITH parsed AS (
  SELECT 
    ...
    initcap(
      CASE
        WHEN POSITION(' ' IN COALESCE(r.data ->> 'Cliente', '')) > 0
          THEN split_part(r.data ->> 'Cliente', ' ', 1)
        ELSE COALESCE(r.data ->> 'Cliente', '')
      END
    ) AS primeiro_nome,
...
enriquecido AS (
  ...
  LEFT JOIN lookup_casa_conta l1 
    ON l1.casa_norm = p.casa AND l1.conta = p.conta 
    AND p.conta IS NOT NULL AND p.conta <> ''
  LEFT JOIN lookup_casa_cpf l2 
    ON l2.casa_norm = p.casa AND l2.cpf = p.cpf_cnpj 
    AND p.cpf_cnpj <> ''
  LEFT JOIN lookup_cpf_qualquer l3 
    ON l3.cpf = p.cpf_cnpj 
    AND p.cpf_cnpj <> ''
),
classificado AS (
  SELECT 
    e.id_raw, e.ingested_at, e.id_carga, e.data_referencia,
    e.casa, e.conta, e.documento_formatado, e.cpf_cnpj,
    e.cliente_nome_original, e.cliente_nome, e.primeiro_nome,
    e.produto, e.d0, e.d_mais_1, e.d_mais_2, e.d_mais_3, e.total_saldo,
    normalize_banker(
      COALESCE(e.banker_1, e.banker_2, e.banker_3, e.banker_raw)
    ) AS banker,
    normalize_advisor(
      COALESCE(e.advisor_1, e.advisor_2, e.advisor_3, e.advisor_raw)
    ) AS advisor,
...
SELECT 
  id_raw, id_carga, data_referencia, ingested_at,
  conta, documento_formatado, cpf_cnpj,
  cliente_nome_original, cliente_nome, primeiro_nome,
  tipo_cliente, casa, produto, d0, d_mais_1, d_mais_2, d_mais_3,
  total_saldo, banker, advisor, finder, canal,
