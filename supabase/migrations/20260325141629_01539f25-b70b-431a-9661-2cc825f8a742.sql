...
DECLARE
  v_perfil_id uuid;
BEGIN
  SELECT id INTO v_perfil_id FROM perfis_acesso WHERE nome = COALESCE(p_perfil_nome, p_role) LIMIT 1;
  ...
  RETURN json_build_object('success', true, 'message', 'Usuario salvo com sucesso.');
