...
-- Create a function that deletes unauthorized users from auth.users
-- Called by handle_new_user when domain is not allowed
-- Uses a deferred approach: marks user as banned so session is invalidated
CREATE OR REPLACE FUNCTION public.ban_unauthorized_user(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  -- Ban the user in auth.users so their session token becomes invalid
  UPDATE auth.users 
...
-- Update handle_new_user to ban unauthorized domain users
...
BEGIN
  ...
  -- Scenario C: Domain NOT allowed → ban user + notify admin
  ELSE
    -- Ban the user so their session/token is invalidated
    PERFORM ban_unauthorized_user(NEW.id);

    INSERT INTO notificacoes_admin (tipo, titulo, mensagem, dados)
    VALUES (
      'dominio_nao_autorizado',
      'Tentativa de acesso com domínio não autorizado',
      'O usuário ' || v_nome_usuario || ' (' || NEW.email || ') tentou acessar com domínio não autorizado: ' || v_email_domain || '. Usuário foi banido automaticamente.',
      jsonb_build_object('email', NEW.email, 'nome', v_nome_usuario, 'user_id', NEW.id::text, 'dominio', v_email_domain)
    );
  END IF;
