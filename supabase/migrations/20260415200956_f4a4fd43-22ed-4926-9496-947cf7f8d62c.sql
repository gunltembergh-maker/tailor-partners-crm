...
BEGIN
  ...
  ELSIF v_domain_allowed THEN
    ...
    VALUES (
      ...
      'O usuário ' || v_nome_usuario || ' (' || NEW.email || ') com domínio ' || v_email_domain || ' solicitou acesso ao Hub. Aguardando aprovação.',
      jsonb_build_object('email', NEW.email, 'nome', v_nome_usuario, 'dominio', v_email_domain, 'timestamp', now()::text)
    );
  ...
  ELSE
    ...
    VALUES (
      ...
      'Tentativa de acesso bloqueada do email ' || NEW.email || ' com domínio não autorizado: ' || v_email_domain,
      jsonb_build_object('email', NEW.email, 'dominio', v_email_domain, 'timestamp', now()::text)
    );
  END IF;
