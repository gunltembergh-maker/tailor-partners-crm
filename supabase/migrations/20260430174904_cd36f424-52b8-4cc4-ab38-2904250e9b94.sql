...
-- NOVO: lookup específico do Advisor por (casa, documento), priorizando nomes reais
...
-- Fallback: lookup do Advisor por documento em qualquer casa (mesmo critério de prioridade)
lookup_advisor_doc_qualquer AS (
  SELECT cpf,
    COALESCE(
      (array_agg(advisor) FILTER (
        WHERE advisor IS NOT NULL
  ...
  SELECT p.*,
    ...
    la1.advisor AS advisor_doc_casa,
    la2.advisor AS advisor_doc_qualquer
  FROM parsed p
  ...
  LEFT JOIN lookup_advisor_casa_doc la1 
    ON la1.casa_norm = p.casa AND la1.cpf = p.cpf_cnpj AND p.cpf_cnpj <> ''
  LEFT JOIN lookup_advisor_doc_qualquer la2
  ...
  SELECT e.id_raw, e.ingested_at, e.id_carga, e.data_referencia, e.casa, e.conta,
    ...
    -- Advisor: SEMPRE prioriza lookup por (casa, documento) na Base Consolidada
    normalize_advisor(COALESCE(e.advisor_doc_casa, e.advisor_doc_qualquer, e.advisor_1, e.advisor_2, e.advisor_3, e.advisor_raw)) AS advisor,
