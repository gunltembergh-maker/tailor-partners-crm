...
-- ═══════════════════════════════════════════════════════════════
-- Add p_casa filter to ALL captação and contas RPCs
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  RETURN QUERY
  SELECT c.anomes,
    to_char(to_date(c.anomes::text,'YYYYMM'),'Mon/YY'),
    c.tipo_captacao, SUM(c.captacao)
...
BEGIN
  RETURN QUERY
  SELECT
    (SELECT COUNT(DISTINCT data->>'Conta')
     ...
     WHERE lower(data->>'Tipo') LIKE '%igra%'
       ...
       AND (p_casa IS NULL OR data->>'Casa' = ANY(p_casa))),
...
BEGIN
  RETURN QUERY
  SELECT bc.anomes, to_char(to_date(bc.anomes::text,'YYYYMM'),'Mon/YY'),
    bc.tipo, bc.casa, COUNT(*)
...
-- Drop old overloads without p_casa (6 params) to avoid ambiguity
DROP FUNCTION IF EXISTS public.rpc_captacao_kpis(integer[], text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.rpc_captacao_agg_mes(integer[], text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.rpc_captacao_treemap(integer[], text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.rpc_contas_kpis(integer[], text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.rpc_contas_agg_mes(integer[], text[], text[], text[], text[], text[]);
DROP FUNCTION IF EXISTS public.rpc_contas_total_por_tipo(integer[], text[], text[], text[], text[], text[]);
