...
-- Lookup Advisor por (casa, documento) — só nomes REAIS (filtra Sem Advisor/Legado)
lookup_advisor_casa_doc_real AS (
...
-- Lookup Advisor por documento em qualquer casa — só nomes REAIS
lookup_advisor_doc_real AS (
  SELECT cpf,
    (array_agg(advisor) FILTER (
      WHERE advisor IS NOT NULL
        AND btrim(advisor) <> ''
...
-- Fallback: Advisor "Legado" por documento
lookup_advisor_doc_legado AS (
  SELECT DISTINCT cpf, 'Legado'::text AS advisor
  FROM depara_normalizado
  WHERE cpf <> '' AND lower(btrim(COALESCE(advisor,''))) = 'legado'
), enriquecido AS (
  SELECT p.*,
    ...
    la1.advisor AS advisor_real_casa,
    la2.advisor AS advisor_real_qualquer,
    la3.advisor AS advisor_legado
  FROM parsed p
  ...
  LEFT JOIN lookup_advisor_casa_doc_real la1 
    ON la1.casa_norm = p.casa AND la1.cpf = p.cpf_cnpj AND p.cpf_cnpj <> ''
  LEFT JOIN lookup_advisor_doc_real la2
    ON la2.cpf = p.cpf_cnpj AND p.cpf_cnpj <> ''
  LEFT JOIN lookup_advisor_doc_legado la3
    ON la3.cpf = p.cpf_cnpj AND p.cpf_cnpj <> ''
), classificado AS (
  SELECT e.id_raw, e.ingested_at, e.id_carga, e.data_referencia, e.casa, e.conta,
    ...
    -- Advisor: prioriza nome REAL por (casa, doc) → nome REAL por doc qualquer casa → Legado → fallbacks antigos
    normalize_advisor(COALESCE(
      e.advisor_real_casa,
      e.advisor_real_qualquer,
      e.advisor_legado,
      e.advisor_1, e.advisor_2, e.advisor_3, e.advisor_raw
    )) AS advisor,
